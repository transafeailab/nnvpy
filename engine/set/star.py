...
class Star(object):
    ...
    def __init__(self, Eq, Ineqs):
        ...
        if isinstance(Ineqs, np.ndarray):
            if Ineqs.shape[1] != Eq.shape[1]:
                raise Exception("Inconsistency between Equality and Inequality")
        self.Eq = Eq
        self.Ineqs = Ineqs
        self.Dim = Eq.shape[0]
    ...
    def set_bounds(self, lb, ub):
        ...
        assert isinstance(lb, np.ndarray), 'error: lower bound is not an ndarray'
        assert isinstance(ub, np.ndarray), 'error: upper bound is not an ndarray'
        assert lb.shape[0] == ub.shape[0], 'error: inconsistency between lower bound and upper bound'
    ...
    def affine_map(self, W, b):
        ...
        assert isinstance(W, np.ndarray),'error: mapping matrix should be an ndarray'
        assert W.shape[1] == self.Dim, 'error: inconsistency between the mapping matrix and the Star'
        if b is None:
            ...
            if isinstance(b, np.ndarray):
                if b.shape[0] != W.shape[0]:
                    raise Exception('Inconsistency between mapping matrix and mapping (offset) vector')
                else:
